...
let itemsStore: TrueOwnerItem[] = [
  {
    ...
    challengeQuestions: ["What sticker is near the touchpad and what color is the keyboard skin?"],
  ...
  },
  {
    ...
    challengeQuestions: ["What initials or emblem are embossed on the inside fold?"],
...
export async function POST(req: NextRequest) {
  try {
    ...
    const newItem: TrueOwnerItem = {
      ...
      secretFeatures: type === "lost" && secretFeatures?.trim() ? [secretFeatures.trim()] : undefined,
      challengeQuestions: type === "found" && challengeQuestion?.trim() ? [challengeQuestion.trim()] : undefined,
